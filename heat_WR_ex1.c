...
#define MAX_VARIANCE_TO_MEAN_RATIO 0.0001 // maximum variance to mean ratio to stop the simulation
#define DEEP_OF_DOMAIN 10.0 // deep of domain in meters
#define WIDTH_OF_DOMAIN 10.0 // width of domain in meters
#define NUMBER_OF_POINTS_IN_X 12000
#define NUMBER_OF_POINTS_IN_Y 12000
#define MAX_NUMBER_OF_LOOPS 150// maximum number of loops
...
#define TYPE_CABLE_ARRANGEMENT 1 // 1 for horizontal arrangement, 2 for trifoil arrangement
...
double k_soil1 = 5.0; // thermal conductivity of soil 1 in W/(m.K) (resistivity = 8)- fuente: https://link.springer.com/article/10.1007/s10765-022-03119-5
...
double k_soil2 = 3.0; // thermal conductivity of soil 1 in W/(m.K) (resistivity = 3) - fuente: https://link.springer.com/article/10.1007/s10765-022-03119-5 
...
double Tbottom = 25.0; // temperature at the bottom of the domain
double Tleft   = 25.0; // temperature at the left of the domain
double Tright  = 25.0; // temperature at the right of the domain
double Tini    = 25.0; // initial temperature of the domain
...
int calculate_probabilities( int i, int j) {
    ...
    if (K_P == K_W && K_P == K_E && K_P == K_S && K_P == K_N) {
        ...
        // Coefficients of the temperature terms of the heat equation at interior node, following Noaeimi(2019) paper
        // A_e = h*h ,and delta_{PE} = h, 
        C_E = k_e/a_P ; 
        C_W = k_w/a_P ; 
        C_N = k_n/a_P ; 
        C_S = k_s/a_P ; 
...
int main(void)
{
    ...
    double Ic1 = 1365; // current in cable
    ...
    F_c1 = (33.45/(3.1416*cond_diam*cond_diam/4))*h/ a_P ; // heat source in cable W/m3 --> 1.05 is for insulation losses
    ...
    double Ic2 = 0; // current in cable
    int icable2= (int) (x_cable21/h); // cable number
    int jcable2= (int) (y_cable21/h); // cable number
    F_c2 = 1.05* Ic2 * Ic2 * res_cond/(cable_area)*h/a_P ; // heat source in cable W/m3 --> 1.05 is for insulation losses
    ...
    F_c3 = 1.0*Ic3*Ic3*res_cond/(cable_area)*h/a_P ; // heat source in cable W/m3
