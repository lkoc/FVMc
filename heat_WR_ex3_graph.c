...
#define NUMBER_OF_POINTS_IN_X 100 //5000
#define NUMBER_OF_POINTS_IN_Y 100 //5000
...
#define BACKFILL_HEIGHT_LAYER 1.04 // 1.04 // height of backfill soil layer in m
...
int main(void)
{
    ...
    double y_point = y_cable11 ; // - cable_diam/2-0.002; // y coordinate of the point
    ///////////////////////////////////////
    ///////////////////////////////////////
    ...
    double Ic2 = 328.6; // current in cable
    ...
    double T_walkers = 0.0; // temperature of the walkers
    ...
    ////////////////////////////////////////////////
    // loop for calculate a sub-zone of temperature //
    // points and save in in VTK format            //
    ////////////////////////////////////////////////

    // Define x_ini, x_end, y_ini and y_end (in meters)
    double x_ini = x_cable11 -0.15 ;
    double x_end = x_cable11 +0.15;
    double y_ini = y_cable11 -0.15;
    double y_end = y_cable11 +0.15;
    // Define the number of points in x and y
    int nx_points = 30;
    // resolution of the points in x and y are the same
    double dx = (x_end - x_ini)/(nx_points-1);
    double dy = dx;
    int di = (int) (dx/hmin); // number of steps in x
    int dj = (int) (dy/hmin); // number of steps in y
    int i_ini = (int) (x_ini/hmin); // initial i coordinate
    int j_ini = (int) (y_ini/hmin); // initial j coordinate
    int i_end = (int) (x_end/hmin); // end i coordinate
    int j_end = (int) (y_end/hmin); // end j coordinate

    // loop over the points in x and y
    double average_temperature;
    double x_g, y_g; // coordinates of the point
    #pragma omp parallel private (average_temperature) 
    {
    #pragma omp for nowait    
    for(int i_g= i_ini; i_g<i_end;i_g=i_g+di) {
        for(int j_g = j_ini; j_g<j_end; j_g=j_g+dj) {
            double sum_temperature = 0.0;
            double Sum_Temp = 0.0;
            double sum_squared_temperature = 0.0;
            average_temperature=0.0;
            // loop over the walk length for multi walkers
            for(int n_loops = 0; n_loops < MAX_NUMBER_OF_LOOPS; n_loops++) {
                // start #pragma parallel region
                sum_temperature = 0.0;
                sum_squared_temperature = 0.0;
                    
                        for(int k = 0; k < n_walkers; k++) {
                            T_walkers = single_walk(i_g, j_g);
            ...
            } // close walker outer for 
            average_temperature = (Sum_Temp) / ((double) n_walkers * (double) MAX_NUMBER_OF_LOOPS);
            x_g = (double) i_g * hmin;
            y_g = (double) j_g * hmin;
            // print the temperature of the point
            ...
            fp = fopen("results_graph_ex3.csv", "a"); // open file to append
            fprintf(fp,"%.6f,%.6f,%.6f\n", x_g, y_g, average_temperature);
            printf("%.6f,%.6f,%.6f\n", x_g, y_g, average_temperature);
            fclose(fp);
