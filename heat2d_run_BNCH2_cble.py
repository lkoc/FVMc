...
# Function to mask the entire 2d domain for create variable fields: SP_soil1, double SP_soil2, double SP_xlpe ans double SP_cond as np.float32 type
def mask_domain(ni, nj, SP_soil1, SP_soil2, SP_xlpe, SP_cond):
        
    x = np.arange(0, ni)
    y = np.arange(0, nj)
    # define cable coordinates in the domain (x,y) 
    cx = 0.5*ni
    cy = 0.5*nj
    
      
    
    mask = ((x[np.newaxis, :]*delta_x-cx)**2 + (y[:, np.newaxis]*delta_y-cy)**2) < r**2
...
kr = np.double(0.833)   # W/(m . K)
rho = np.double(2000.0)  # kg/m^3
Cp = np.double(800.0)   # J /( kg . K)
alpha = kr/(rho*Cp)
...
u_top = np.double(20.0)
# Boundary conditions Neumann
u_right = np.double(0.00)
# Boundary conditions Neumman
u_left = np.double(0.0)  # Neumman
# Boundary conditions Neumman
u_bottom = np.double(0.0)
# Initial condition everywhere inside the grid
u_initial = np.double(15.0)

tin = np.full((ni, nj), u_initial).astype(np.float32)

# Usa solución anterior como valor inicial si está disponible
try:
    # pass
