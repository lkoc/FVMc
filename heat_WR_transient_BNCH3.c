...
#define NUMBER_OF_POINTS_IN_X 100 // 7500
#define NUMBER_OF_POINTS_IN_Y 100 //7500
...
#define TMAX 100000000 // maximum temperature of the domain 
...
// Point to calculate the temperature
 ///////////////////////////////////////
 ///////////////////////////////////////
double x_point = 0.9; //x_cable21;          //   L/2; // x coordinate of the point
double y_point = 0.1; //y_cable21 ; // - cable_diam/2-0.002; // y coordinate of the point
...
int calculate_probabilities( int i, int j,  double d_t) {
    ...
    float kw = 0.5*(K_P + K_W);
    float ke = 0.5*(K_P + K_E);
    float ks = 0.5*(K_P + K_S);
    float kn = 0.5*(K_P + K_N);    

    F_c = (d_t / (hmin*hmin)) / (rho_P * Ce_P) ;
    p_P = 1.0 - (4.0*K_P* F_c);
    p_W = 1.0 - (3.0*K_P - ke/2.0 + kw/2.0)*F_c;  // probability to move to the left
    p_E = 1.0 - (2.0*K_P* F_c); // probability to move to the right
    p_N = 1.0 - (1.0*K_P - kn/2.0 + ks/2.0)*F_c;; // probability to move to the top
